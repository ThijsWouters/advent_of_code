defmodule AdventOfCode.Day8 do
  def solve do
    total_number_of_chars = String.length(code_chars)
    chars_in_memory = Enum.map(separate_strings, &String.length/1) |> Enum.sum
    total_number_of_chars - chars_in_memory
  end
  ...
  def solve_part_2 do
    ...
    |> String.replace(~s(\\), ~S(\\))
    |> String.replace(~S("), ~S(\"))
    String.length(new_code) + length(separate_strings) * 2 - String.length(code_chars)
  end

  defp separate_strings do
    [
      "\xa8br\x8bjr\"",
      "nq",
